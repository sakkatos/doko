...
func TestGetVector(t *testing.T) {
	...
	if vec.x != 0 || vec.y != -1 {
		t.Errorf("Vector to up value is expected x = %v, y = %v", 0, -1)
	}

	vec = game.getVector(1)
	if vec.x != 1 || vec.y != 0 {
		t.Errorf("Vector to right value is expected x = %v, y = %v", 0, -1)
	}

	vec = game.getVector(2)
	if vec.x != 0 || vec.y != 1 {
		t.Errorf("Vector to down value is expected x = %v, y = %v", 0, -1)
	}

	vec = game.getVector(3)
	if vec.x != -1 || vec.y != 0 {
		t.Errorf("Vector to left value is expected x = %v, y = %v", 0, -1)
	}
}
